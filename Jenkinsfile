...
pipeline {
    ...
    stages {
        ...
        stage('Run terraform') {
            steps {
                dir('Terraform') {                
                    git branch: 'main', url: 'https://github.com/Robert89111911/Terraform'
                    withAWS(credentials:'AWS', region: 'us-east-1') {
                            sh 'terraform init -backend-config=bucket=robert-tarnawski-panda-devops-core-16'
                            sh 'terraform apply -auto-approve -var bucket_name=robert-tarnawski-panda-devops-core-16'
